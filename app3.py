...
def admin_registration_step1():
    if request.method == 'POST':
       session['reg_name'] = request.form.get('name')
       session['reg_id'] = request.form.get('admin_id')
       session['reg_contact'] = request.form.get('contact')
       session['reg_email'] = request.form.get('email')

       return redirect(url_for('admin_registration_step2'))
    
    return render_template('AdminRegistration1.html')



@app.route('/register/admin/step-2', methods=['GET', 'POST'])
def admin_registration_step2():
    if request.method == 'POST':
        ...
        try:
            ...
            db.session.commit()
            return redirect(url_for('admin_registration_step3'))
        except Exception as e:
            db.session.rollback()
            return f"Database Error: {e}"
    
    return render_template('AdminRegistration2.html')

@app.route('/register/admin/success')
def admin_registration_step3():
    session.clear() # Clear everything after success
    return render_template('AdminRegistration3.html')
